...
def run_eda():
    st.subheader('데이터 분석')
    car_df = pd.read_csv(
        'data/Car_Purchasing_Data.csv', encoding='ISO-8859-1')

    # 라디오 버튼을 이용해서, 데이터 프레임과, 통계치 중에 선택하여
    # 보여줄 수 있도록 만든다.
    ...
    selected_list = st.multiselect('원하는 컬럼 선택', col_list)
    # print(selected_list)
    ...
    ### he라고 넣으면, he가 이름에 들어있는 고객들의 데이터를
    ### 가져옵니다.

    # 1. 유저에게 검색어를 입력받는다.
    word = st.text_input('이름을 검색할 단어를 입력하세요.')
    # 2. 검색어를 고객이름 컬럼에 들어있는 데이터 가져온다.
    result = car_df.loc[car_df.iloc[:,0].str.lower()
                        .str.contains(word.lower()), ]
    # 3. 화면에 보여준다.
    st.dataframe(result)